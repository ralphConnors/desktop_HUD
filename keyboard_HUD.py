...
def update_state(): # Button event
    ...
    for key_name, label in key_label:
        try:
            ...
            label.config(bg=light_color if is_pressed else "#2c3e50")
        except:
            pass
...
def update_mouse_state(button, pressed):
...
def keyset_frame(keyset, frame):
    for key, key_name, row, column in keyset: 
        frameKey = Label(frame, text=key, font=font_setting, bg=bg_color, fg=fg_color, padx=14, pady=10)
        ...
        key_label.append((key_name, frameKey))
...
for frame, keyset in [
    ...
    keyset_frame(keyset, frame)


for mouseBtnName, mouse_name in keyset_4:
    frameMouse = Label(frame4, text=mouseBtnName, font=font_setting, bg=bg_color, fg=fg_color, padx=14, pady=10)
